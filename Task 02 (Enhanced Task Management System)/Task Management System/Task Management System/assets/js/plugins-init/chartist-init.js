 ...
 var dzChartlist = function(){
	...
	var animatingDonutChart = function(){
		...
		var chart = new Chartist.Pie('#animating-donut', {
			series: [
				{ value: 100, meta: "Total Present" },
				{ value: 30, meta: "Total Absent" },
				{ value: 3, meta: "Total Leave" }
			]
		}, {
			donut: true,
			showLabel: false,
	...
	var simplePieChart = function(){
	 ...
	 var pieChart = function () {
		 ...
		 fetch("/api/dashboard/GetStatusBreakdown")
			 .then(res => {
			 ...
			 .then(res => {
				 ...
				 var inProgress = res.InProgress ?? 0;
				 var completed = res.Completed ?? 0;

				 var pendingPer = (res.PendingPercent ?? 0).toFixed(0) + '%';
				 var inProgressPer = (res.InProgressPercent ?? 0).toFixed(0) + '%';
				 var completedPer = (res.CompletedPercent ?? 0).toFixed(0) + '%';
				 ...
				 var options = {
					 donut: true,
					 donutWidth: 60,
					 showLabel: false   // IMPORTANT (no hide issue now)
				 };
				 ...
				 // Legend (RIGHT SIDE with matching colors)
				 document.getElementById("chart-legend").innerHTML = `
                <li><span style="color:#f39c12;">●</span> Pending (${pendingPer})</li>
                <li><span style="color:#3498db;">●</span> In Progress (${inProgressPer})</li>
                <li><span style="color:#2ecc71;">●</span> Completed (${completedPer})</li>
            `;
			 })
			 .catch(err => {
